...
int main() {
	...
	do {
		mainMenu = std::move(std::unique_ptr<mainMenu_t>(new mainMenu_t(window.get(), levelName, difficulty)));
		mainMenu->makeMenu();
		mainMenu->action();
		do {
		GAME = std::unique_ptr<game_t>(new game_t(window.get(), levelName, difficulty));
		GAME->start();	
